...
---Advances the spinner every 100ms (or just once if precise time measurement is not supported) and returns the new
---spinner symbol.
function M:__advance_spinner()
  if vim.uv then
    self.__spinner_index = math.floor(vim.uv.hrtime() / 1e8)
  else
